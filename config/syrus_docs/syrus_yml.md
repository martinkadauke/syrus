...
`enabled` explicitly turns visual review on (`true`) or off (`false`) for this repository, overriding the instance-wide default. When the block (or the `enabled` key within it) is omitted, no repository-level override is recorded and the instance-wide `AppSetting.visual_review_enabled?` default applies instead — so a repository can set `rounds`, `when_files_changed`, or `seed_notes` without omitted `enabled` forcing the feature off.
