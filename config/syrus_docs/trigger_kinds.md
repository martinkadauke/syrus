...
`main_grader` is exempt from user Job start gates and the global agent-run concurrency deferral that would make the health signal stale. It starts and runs even while another Job in the repository has urgent priority, and it is also exempt from the broken-main gate because the workflow is the check that measures main-branch grader health.
