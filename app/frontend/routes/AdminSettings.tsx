...
function SettingsView({ payload, onNotice }: { payload: AdminSettingsPayload; onNotice: (message: string | null) => void }) {
  return (
    <>
      <section className="divide-y divide-gray-200 rounded border border-gray-200 bg-white">
...
function SecretRow({ secret, onNotice }: { secret: ClearableSecret; onNotice: (message: string | null) => void }) {
  ...
  return (
    <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <div className="text-sm font-medium text-gray-900">{secret.label}</div>
        <div className={`mt-1 text-xs ${secret.set ? "text-gray-500" : "text-amber-700"}`}>
          {secret.set ? "Currently set. Blank update fields will not clear it." : "Not set."}
        </div>
      </div>
      {secret.set ? (
        <button
          className="self-start rounded bg-red-50 px-3 py-1.5 text-sm font-medium text-red-700 hover:bg-red-100 disabled:cursor-not-allowed disabled:text-red-300 sm:self-auto"
      ...
      {clearSecret.isError ? <div className="text-xs text-red-700" role="alert">{errorMessage(clearSecret.error, "Unable to clear secret.")}</div> : null}
    </div>
  )
}
...
function SettingsForm({ payload, onNotice }: { payload: AdminSettingsPayload; onNotice: (message: string | null) => void }) {
  ...
  return (
    <form className="space-y-4 rounded border border-gray-200 bg-white p-6" onSubmit={submit}>
      <p className="text-sm text-gray-600">Leaving secret fields blank keeps the stored value unchanged. Use the Clear controls above to remove a secret.</p>
      ...
      <div className="flex items-start gap-3">
        ...
        />
        <span>
          <label className="block text-sm font-medium text-gray-700" htmlFor="admin-settings-signups-open">Open signups</label>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700" htmlFor="admin-settings-telegram-bot-token">Telegram bot token</label>
        <p className="mt-1 text-xs font-normal text-gray-500">Optional app-wide fallback for Telegram operator chat. ENV TELEGRAM_BOT_TOKEN still takes precedence.</p>
        <input
          id="admin-settings-telegram-bot-token"
          autoComplete="off"
          className="mt-2 block w-full rounded border border-gray-300 px-3 py-2 text-sm shadow-sm focus:outline-blue-600"
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700" htmlFor="admin-settings-telegram-webhook-secret">Telegram webhook secret</label>
        <p className="mt-1 text-xs font-normal text-gray-500">Optional app-wide fallback for validating Telegram webhook requests. ENV TELEGRAM_WEBHOOK_SECRET still takes precedence.</p>
        <input
      ...
      <button
        className="rounded bg-blue-600 px-3.5 py-2 text-sm font-medium text-white hover:bg-blue-500 disabled:cursor-not-allowed disabled:bg-blue-300"
        disabled={update.isPending}
        type="submit"
      >
        {update.isPending ? "Saving..." : "Save"}
      </button>
      {update.isError ? <p className="text-sm text-red-700" role="alert">{errorMessage(update.error, "Unable to update settings.")}</p> : null}
    </form>
  )
}
