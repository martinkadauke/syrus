...
describe("App", () => {
  ...
  it("shows dashboard filter save controls in the v2 sidebar folders once rows are current", async () => {
    const script = document.createElement("script")
    script.id = "syrus-bootstrap-data"
    script.type = "application/json"
    script.textContent = JSON.stringify(bootstrapPayload({
    ...
    try {
      ...
      await waitFor(() => {
        expect(within(smartFoldersPanel).getByLabelText("Folder name")).toBeInTheDocument()
  ...
  it("shows update and save actions in the sidebar when the active saved dashboard folder filter differs", async () => {
    const script = document.createElement("script")
    script.id = "syrus-bootstrap-data"
    script.type = "application/json"
    script.textContent = JSON.stringify(bootstrapPayload({
  ...
  it("updates the active saved dashboard folder with the applied filter", async () => {
    ...
    try {
      ...
      await waitFor(() => {
        expect(fetchSpy.mock.calls.some(([input, init]) => String(input) === "/api/v1/app/smart_folders/7" && init?.method === "PATCH")).toBe(true)
      })
    } finally {
      script.remove()
    }
  ...
  it("renders a chat and sends a message from the app API", async () => {
    ...
    expect(screen.getByText("Discuss aqueducts.").closest(".chat-prose")?.parentElement).toHaveClass("sm:dark:bg-gray-900", "sm:dark:border-gray-700")
    ...
    results: [
      {
        ...
        top_matches: [
          chatSearchMatch({ message_id: 11, role: "assistant", snippet: "Best <b>needle</b> <i>ignored</i>", created_at: threeDaysAgo }),
          chatSearchMatch({ message_id: 13, role: "assistant", snippet: "Third <b>needle</b>", created_at: threeDaysAgo }),
          chatSearchMatch({ message_id: 14, role: "tool_result", snippet: "Fourth <b>needle</b>", created_at: threeDaysAgo })
        ],
        total_match_count: 4,
