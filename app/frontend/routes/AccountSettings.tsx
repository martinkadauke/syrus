...
function CredentialsView({ payload, onNotice, section }: { payload: CredentialsPayload; onNotice: (message: string | null) => void; section: AccountSettingsSection }) {
  ...
  return (
    <>
      ...
      {section === "credentials" ? <HiddenChatsPanel onNotice={onNotice} /> : null}
  ...
  return (
    <section className="rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">Hidden chats</h2>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Restore chats hidden from the sidebar and chat search.</p>
        </div>
      </div>
            ...
            <div className="divide-y divide-gray-200 dark:divide-gray-700">
              {chats.map((chat) => (
                <div className="grid gap-3 px-4 py-3 text-sm sm:grid-cols-[1fr_auto] sm:items-center" key={chat.id}>
                  ...
                  <button
                    className="rounded border border-gray-300 dark:border-gray-600 px-3 py-1.5 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:cursor-not-allowed disabled:text-gray-300 dark:disabled:text-gray-600"
