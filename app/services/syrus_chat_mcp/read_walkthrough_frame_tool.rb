...
module SyrusChatMcp
  ...
  # (SyrusChatMcp.image_result). Claude Code renders that into the agent's
  ...
  class ReadWalkthroughFrameTool < MCP::Tool
    ...
    class << self
      def call(server_context:, walkthrough_id: nil, timestamp: nil, **)
        ...
        return SyrusChatMcp.invalid("timestamp must be mm:ss or whole seconds") if seconds.nil?
        ...
        return SyrusChatMcp.invalid(frameless_error(walkthrough, seconds)) if frame.nil?

        SyrusChatMcp.image_result(
          jpeg: frame.jpeg,
