  ...
  class AnalyzeWalkthroughSegmentTool < MCP::Tool
    ...
    class << self
      ...
      def call(server_context:, walkthrough_id: nil, start: nil, focus: nil, **rest)
        ...
        return SyrusChatMcp.invalid("walkthrough not found in this chat: #{walkthrough_id.inspect}") unless walkthrough
        unless walkthrough.analyzed? && walkthrough.analysis.present?
          return SyrusChatMcp.invalid("walkthrough #{walkthrough.id} has no completed analysis to zoom into")
        end
        ...
        if start_seconds.nil? || end_seconds.nil?
          return SyrusChatMcp.invalid("start and end must be mm:ss or whole seconds")
        end

        start_seconds, end_seconds, truncated = clamp(start_seconds, end_seconds, walkthrough.duration_seconds)
        return SyrusChatMcp.invalid("end must be after start, and within the video") if start_seconds.nil?

        unless chat_session.user.gemini_configured?
          return SyrusChatMcp.invalid("Gemini is not configured — add an API key under Credentials.")
        end
        ...
        SyrusChatMcp.success(success)
      rescue StoredVideoUnavailable
        SyrusChatMcp.invalid(
        ...
        SyrusChatMcp.invalid("Gemini rejected the API key: #{e.message} Check it under Credentials.")
      rescue Gemini::Client::RateLimited
        SyrusChatMcp.invalid("Gemini's quota is busy right now (free-tier per-minute limits). Try the segment again in a minute.")
      rescue Gemini::Client::ConnectionError
        SyrusChatMcp.invalid("Could not reach Gemini — try the segment again.")
