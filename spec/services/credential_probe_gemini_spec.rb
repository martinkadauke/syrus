...
RSpec.describe CredentialProbe, "gemini" do
  let(:client) { instance_double(Gemini::Client) }
  let(:captured_keys) { [] }

  around do |example|
    original = described_class.gemini_client_factory
    described_class.gemini_client_factory = lambda do |api_key:|
      captured_keys << api_key
      client
    end
    example.run
  ensure
    described_class.gemini_client_factory = original
  end

  describe ".gemini_key" do
    it "refuses a blank key without calling Google" do
      result = described_class.gemini_key(key: "  ")
    ...
    it "is ok when a video-capable flash model is available" do
      ...
      result = described_class.gemini_key(key: "AIza-unsaved")
    ...
    it "is not ok when the key works but no video-capable model is available" do
      ...
      result = described_class.gemini_key(key: "AIza-restricted")
    ...
    it "reports a rejected key on AuthError" do
      allow(client).to receive(:list_models).and_raise(Gemini::Client::AuthError, "API key not valid.")

      result = described_class.gemini_key(key: "AIza-bad")
    ...
    it "reports throttling on RateLimited" do
      allow(client).to receive(:list_models).and_raise(Gemini::Client::RateLimited, "quota")

      result = described_class.gemini_key(key: "AIza-busy")
    ...
    it "reports an unreachable Google on network errors" do
      ...
      result = described_class.gemini_key(key: "AIza-offline")
  ...
  describe ".call with credential gemini_api_key" do
    it "probes the stored key through the same path" do
      ...
      result = described_class.call(user: user, credential: "gemini_api_key")
