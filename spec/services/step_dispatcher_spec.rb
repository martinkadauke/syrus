...
RSpec.describe StepDispatcher do
  ...
  describe ".advance_from" do
    ...
    it "serializes landing graders when the cap is 1" do
      AppSetting.current.update!(max_concurrent_landing_grader_runs: 1)
      clear_enqueued_jobs
      ...
      expect(graders.map { |grader| grader.reload.runs.count }).to eq([ 1, 0, 0 ])
      expect(collect.reload.runs).to be_empty
      expect(enqueued_jobs.count { |entry| entry[:job] == RunJob }).to eq(1)
      expect(fanout_workflow.reload.artifact("grader_fanout_batches").last).to include(
        ...
        "run_ids" => [ graders.first.runs.last.id ],
        "cap" => 1,
        "active_before" => 0,
        "queue" => "merges"
      )
    ...
    it "fans out landing graders up to the configured cap" do
      ...
      clear_enqueued_jobs
      fanout_workflow = Workflow.create!(job: job, trigger_kind: "merge_train")
      loop_id = SecureRandom.uuid
      fanout = Step.create!(workflow: fanout_workflow, kind: "grader_fanout", position: 0, loop_id: loop_id)
      collect = Step.create!(workflow: fanout_workflow, kind: "grader_collect", position: 4, loop_id: loop_id)
      graders = %w[alpha beta gamma].map.with_index do |name, index|
      ...
      expect(graders.map { |grader| grader.reload.runs.count }).to eq([ 1, 1, 0 ])
      expect(collect.reload.runs).to be_empty
      expect(enqueued_jobs.count { |entry| entry[:job] == RunJob }).to eq(2)
    end

    it "queues the next pending landing grader after a failed grader frees a slot" do
      AppSetting.current.update!(max_concurrent_landing_grader_runs: 2)
      clear_enqueued_jobs
      fanout_workflow = Workflow.create!(job: job, trigger_kind: "auto_merge")
      loop_id = SecureRandom.uuid
      first = Step.create!(workflow: fanout_workflow, kind: "grader", position: 0, loop_id: loop_id, state: "failed", started_at: 3.seconds.ago, finished_at: Time.current)
      ...
      third = Step.create!(workflow: fanout_workflow, kind: "grader", position: 2, loop_id: loop_id)
      collect = Step.create!(workflow: fanout_workflow, kind: "grader_collect", position: 3, loop_id: loop_id)
      first.runs.create!(job: job, trigger_kind: "auto_merge", state: "failed")
      second.runs.create!(job: job, trigger_kind: "auto_merge", state: "running")
      first.update!(next_step_id: second.id)
      second.update!(next_step_id: third.id)
      third.update!(next_step_id: collect.id)

      expect {
        described_class.fail_from(first)
      }.to change { third.runs.count }.by(1)

      expect(collect.reload.runs).to be_empty
    end

    it "counts an in-place worker retry as active before scheduling pending landing graders" do
      AppSetting.current.update!(max_concurrent_landing_grader_runs: 1)
      clear_enqueued_jobs
      fanout_workflow = Workflow.create!(job: job, trigger_kind: "auto_merge")
      loop_id = SecureRandom.uuid
      retrying = Step.create!(workflow: fanout_workflow, kind: "grader", position: 0, loop_id: loop_id)
      pending = Step.create!(workflow: fanout_workflow, kind: "grader", position: 1, loop_id: loop_id)
      done = Step.create!(workflow: fanout_workflow, kind: "grader", position: 2, loop_id: loop_id, state: "succeeded", started_at: 2.seconds.ago, finished_at: Time.current)
      collect = Step.create!(workflow: fanout_workflow, kind: "grader_collect", position: 3, loop_id: loop_id)
      retrying.runs.create!(job: job, trigger_kind: "auto_merge", state: "queued")
      retrying.update!(next_step_id: pending.id)
      pending.update!(next_step_id: done.id)
      done.update!(next_step_id: collect.id)

      expect {
        described_class.advance_from(done)
      }.not_to change { pending.runs.count }
