  ...
  it "prepares the test database before preparing and running the parallel suite" do
    Dir.mktmpdir do |dir|
      ...
      expect(File.read(log_path).lines.map(&:chomp)).to eq([
        ...
        "bundle-exec command=rake args=rake parallel:prepare",
        "bundle-exec command=parallel_rspec args=parallel_rspec -n 8 --quiet --exec-args bin/rspec-worker spec/models/job_spec.rb"
      ])
    end
  end
end
