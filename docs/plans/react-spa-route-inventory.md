...
| `/jobs/:id` | `jobs#show` + `/api/v1/app/jobs/:id`, `/api/v1/app/jobs/:id/timeline` | `spa-core` | App API read model now covers summary metadata, dependencies, attachments, workflow tree, action paths, and split timeline data. React route still needs to replace the ERB page. |
| `/jobs/:id/source` | `jobs#source` | `spa-core` | Source browser. App API now exposes refs/tree/file content at `/api/v1/app/jobs/:id/source`; React route still pending. |
