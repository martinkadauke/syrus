...
| `/jobs/:id` | `spa#show` + `/api/v1/app/jobs/:id`, `/api/v1/app/jobs/:id/timeline`, `/api/v1/app/jobs/:job_id/runs/:run_id/artifacts` | `spa-core` | Migrated to the React Job detail page. Internal repository/dependent-Job links use React Router navigation. Legacy ERB fallback removed; HTML member commands remain until the final command-route cleanup. |
