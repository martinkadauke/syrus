...
| chat commands | `#create_bookmark`, `#add_attachment`, `#destroy_attachment`, `#confirm_proposal`, `#reject_proposal`, `#confirm_pending_action`, `#destroy_pending_action` | `app-api-needed` | Message send/stop/refresh/reset and older-message loading moved to app API with the React chat renderer; convert the remaining Turbo/HTML responses to typed browser JSON. |
